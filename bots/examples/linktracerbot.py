...
def onNewMessage(message, writer, timestamp, room):
    if ("<a href" in message) :
        try:
            url = message.split("\"")[1]
            page = urlopen(url)
            p = parse(page)
            sendMsg(writer+" has sent a link to the page \""+p.find(".//title").text+"\"", "#3498DB", room)
        except Exception as e:
            sendMsg("Could not retrieve link preview. The link may be malformed or missing an https:// prefix.", "#E74C3C", room)
